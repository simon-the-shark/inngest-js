...
export type EventsFromFunction<ID extends string = string, T extends InngestFunction.Any<ID> = InngestFunction.Any<ID>> = T extends InngestFunction<ID, any, any, any, infer IClient extends Inngest<ClientOptions>, any, any> ? GetEvents<IClient, true> : never;
/**
 ...
 */
export type TriggerEventFromFunction<TFunction extends InvokeTargetFunctionDefinition> = TFunction extends InngestFunction.Any ? PayloadForAnyInngestFunction<TFunction["opts"]["id"], TFunction> : TFunction extends InngestFunctionReference<infer IInput extends MinimalEventPayload, any> ? IInput : MinimalEventPayload;
/**
 ...
 * @internal
 */
export type PayloadForAnyInngestFunction<ID extends string, TFunction extends InngestFunction.Any<ID>, TEvents extends Record<string, EventPayload> = TFunction extends InngestFunction.Any<ID> ? EventsFromFunction<ID, TFunction> : never> = TFunction extends InngestFunction<ID, any, any, any, any, any, infer ITriggers extends InngestFunction.Trigger<keyof TEvents & string>[]> ? IsEqual<TEvents[EventNameFromTrigger<TEvents, ITriggers[number]>]["name"], `${internalEvents.ScheduledTimer}`> extends true ? object : Simplify<Omit<TEvents[EventNameFromTrigger<TEvents, ITriggers[number]>], "name" | "ts">> : never;
